...
def add_index(img, indice):
    ...
    scaled = img.divide(10000).copyProperties(img, img.propertyNames())

    if indice == "NDVI":
        index = scaled.normalizedDifference(["B8", "B4"]).rename("NDVI")
    elif indice == "NDRE":
        index = scaled.normalizedDifference(["B8", "B6"]).rename("NDRE")
    else:
        index = scaled.normalizedDifference(["B8", "B11"]).rename("NDMI")

    return scaled.addBands(index)
