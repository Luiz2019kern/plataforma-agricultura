...
if st.button("Analisar melhor data do índice"):
    ...
    if "gdf_prod_filtrado" not in st.session_state:
        ...
        for i in range(min(20, lista_imagens.size().getInfo())):

            img = ee.Image(lista_imagens.get(i))
            data = img.date().format("YYYY-MM-dd").getInfo()

            valores = img.select(indice).sampleRegions(
                collection=pontos_ee,
                scale=10,
                geometries=False
            )
            ...
            if indice in df.columns and col_prod in df.columns:

                corr = df[[col_prod, indice]].corr().iloc[0, 1]
