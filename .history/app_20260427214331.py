        ...
        if len(col_numericas) == 0:
            ...
            if st.button("Extrair índice nos pontos"):
                if "img_index" not in st.session_state:
                    ...
                    # Limite temporário para evitar erro do Earth Engine/getInfo()
                    if len(gdf_pts) > 5000:
                        st.warning(
                            f"Foram encontrados {len(gdf_pts)} pontos. "
                            "Usando amostra de 5000 pontos para evitar limite do Earth Engine."
                            )
                        gdf_pts = gdf_pts.sample(
        n=5000,
        random_state=42
    )
                    ...
                    df_out = df_out.dropna(subset=[indice_usado, col_prod])
                    ...
                    if indice_usado in df_out.columns:
                        ...
                        # Reanexa valores do índice ao GeoDataFrame original por ordem.
                        # Para protótipo, assume que a ordem dos pontos retornados foi preservada.
                        gdf_indice = gdf_pts.iloc[df_out.index].copy()
                        gdf_indice[indice_usado] = df_out[indice_usado].values

                        st.session_state["gdf_prod_com_indice"] = gdf_indice
# Interpola o índice com a mesma resolução/potência escolhidas para a produtividade.
                        ...
                        st.success("Índice extraído e interpolado com sucesso.")
                    else:
